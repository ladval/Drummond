...
For $i = 0 To $iUniqueElements Step +1
	Local $IdDimUnique = $aTRK_DataUnique[$i]
	Local $aMemoryArrayTest[Ubound($aTRK_DataUnique,1)][Ubound($aTRK_Data,2)]
	For $j = 0 To UBound($aTRK_Data) - 1 Step +1
		Local $IdDim = $aTRK_Data[$j][0]
		If $IdDim == $IdDimUnique Then
			Local $aExtractedTRK_Data = _ArrayExtract($aTRK_Data, $j, $j)
			_ArrayAdd($aMemoryArrayTest,$aTRK_Data)
			ExitLoop
		EndIf
	Next
Next
_ArrayDisplay($aMemoryArrayTest,'$aMemoryArrayTest')
