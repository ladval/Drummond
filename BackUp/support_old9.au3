...
Local $a = ReadCSV(@ScriptDir & "\Libro1.csv")


For $i = 0 To UBound($a) - 1 Step +1
	Local $sInvoiceNumber = $a[$i][0]
	Local $sPedido = $a[$i][1]
	Local $sJefeCuenta = $a[$i][2]
	Local $sProveedor = $a[$i][3]
	Local $sTMR = $a[$i][4]
	...
	Json_Dump($sJsonFactData)
	Local $oJsonFactData = Json_Decode($sJsonFactData)
	Local $sInvoiceState_8 = Json_ObjPut($oJsonFactData, '.AdditionalProperty[8].Value',$sInvoiceNumber)
	Local $sInvoiceState_7 = Json_ObjPut($oJsonFactData, '.AdditionalProperty[7].Value',$sPedido)
	Local $sInvoiceState_14 = Json_ObjPut($oJsonFactData, '.AdditionalProperty[14].Value',$sJefeCuenta)
	Local $sInvoiceState_3 = Json_ObjPut($oJsonFactData, '.AdditionalProperty[3].Value',$sProveedor)
	Local $sInvoiceState_13 = Json_ObjPut($oJsonFactData, '.AdditionalProperty[13].Value',$sTMR)
	Json_Dump($oJsonFactData)

	

	;~ ConsoleWrite($sInvoiceNumber & @CRLF)
	;~ ConsoleWrite($sPedido & @CRLF)
	;~ ConsoleWrite($sJefeCuenta & @CRLF)
	;~ ConsoleWrite($sProveedor & @CRLF)
	;~ ConsoleWrite($sTMR & @CRLF)
	;~ ConsoleWrite(@CRLF)
	Exit
Next
