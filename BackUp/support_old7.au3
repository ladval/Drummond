...
For $i = 0 To UBound($a) - 1 Step +1
	...
	Local $sSQLInvoiceDataQuery = "SELECT * FROM [BotAbc].[dbo].[tfact_ApiProcesos] WHERE InvoiceNumber = '" & $sInvoiceNumber & "' AND Empresa = '860536003'"
	Local $aSQLInvoiceDataQuery = _ModuloSQL_SQL_SELECT($sSQLInvoiceDataQuery)
	_ArrayDisplay($aSQLInvoiceDataQuery,'$aSQLInvoiceDataQuery')
